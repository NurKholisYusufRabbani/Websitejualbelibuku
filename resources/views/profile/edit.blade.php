...
@section('content')
    ...
    <div class="flex flex-col items-center mb-6 mt-10">
        ...
        <!-- Tombol Keranjang dan Top Up -->
        <div class="grid grid-cols-2 justify-center mt-4 gap-x-6">
            <!-- Cart -->
            <a href="{{ route('cart.index') }}" class="flex items-center justify-center w-48 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">
                <i class="fas fa-shopping-cart mr-2"></i> Keranjang
            </a>

            <!-- Top Up (Coming Soon) -->
            <span class="flex items-center justify-center w-48 px-4 py-2 bg-gray-500 text-white rounded-lg cursor-not-allowed">
            ...
            <div class="bg-white dark:bg-gray-800 p-6 shadow rounded-lg">
                ...
                <form method="POST" action="{{ route('profile.update') }}">
                    ...
                    <div class="mt-4">
                        ...
                        <p class="px-4 py-2 border rounded-lg dark:bg-gray-700 dark:text-white">{{ auth()->user()->nama ?? '' }}</p>
                    </div>
                    <div class="mt-4">
                        <label class="block text-gray-700 dark:text-white">Email</label>
                        <p class="px-4 py-2 border rounded-lg dark:bg-gray-700 dark:text-white">{{ auth()->user()->email ?? '' }}</p>
                    </div>
                </form>
            </div>
            <!-- Ubah Password -->
